...
const RCV = (() => {
  ...
  const D5_CX  = 380, D5_CY = 262, D5_R = 14;  // 5D center; directional offset = 28
  ...
  function buildRCBody(g) {
    ...
    g.appendChild(el('rect', {
      x: 22, y: 18, width: W - 44, height: H - 34, rx: 42,
      fill: '#14142a', stroke: '#2a2a55', 'stroke-width': 2
    }));
    // Grip indents (cosmetic)
    for (const [gx, type] of [[22, -1], [W - 22, 1]]) {
      const path = type === -1
  ...
  function buildTrigger(g, cfg, id, huLabel) {
    const r = el('rect', {
      x: cfg.x, y: cfg.y, width: cfg.w, height: cfg.h, rx: cfg.rx,
      fill: '#1a1a38', stroke: '#3a3a75', 'stroke-width': 1.5
    });
    g.appendChild(r);
    refs[id + '_rect'] = r;
    ...
    if (id === 'rtrig') {
      const div = el('line', {
        x1: cfg.x + cfg.w / 2, y1: cfg.y + 4,
        x2: cfg.x + cfg.w / 2, y2: cfg.y + cfg.h - 4,
        stroke: '#2e2e5e', 'stroke-width': 1
      });
      g.appendChild(div);
      const lhalf = el('rect', {
        x: cfg.x, y: cfg.y, width: cfg.w / 2, height: cfg.h, rx: cfg.rx,
        fill: 'none', stroke: 'none', opacity: 0.75
      });
      g.appendChild(lhalf);
      refs['rtrig_half_rect'] = lhalf;
    }
    g.appendChild(text(huLabel, {
      x: cfg.x + cfg.w / 2, y: cfg.y + cfg.h / 2 + 5,
      'text-anchor': 'middle', fill: '#4a4a88', 'font-size': 11,
  ...
  function buildWheel(g, cfg, id, label) {
    ...
    g.appendChild(el('circle', {
      ...
      fill: '#0d0d22', stroke: '#2a2a5a', 'stroke-width': 2
    }));
    // Wheel arc indicator (arc segment showing position)
    const arc = el('path', {
      d: wheelArcPath(cfg, 0),
      fill: 'none', stroke: '#3a3a80', 'stroke-width': 5,
    ...
    g.appendChild(text(label, {
      x: cfg.cx, y: cfg.cy + 4,
      'text-anchor': 'middle', fill: '#3a3a70', 'font-size': 9,
      'font-family': 'monospace'
    }));
  }
  ...
  function buildButton(g, cfg, id, label, isPico = false) {
    ...
    const c = el('circle', { cx: cfg.cx, cy: cfg.cy, r: cfg.r, fill, stroke, 'stroke-width': 1.5 });
    isPico && c.classList.add('rc-pico-dim');
    g.appendChild(c);
    refs[id + '_btn'] = c;
    const t = text(label, {
      x: cfg.cx, y: cfg.cy + 4,
      'text-anchor': 'middle', fill: isPico ? '#404070' : '#4a4a88',
      'font-size': 10, 'font-family': 'monospace', 'pointer-events': 'none'
    });
    g.appendChild(t);
    refs[id + '_lbl'] = t;
  }
  ...
  function buildStick(g, cfg, id, label) {
    ...
    g.appendChild(el('circle', {
      cx: cfg.cx, cy: cfg.cy, r: cfg.r - 4,
      fill: 'none', stroke: '#1e1e40', 'stroke-width': 1, 'stroke-dasharray': '4 3'
    }));
    // Stick dot
    const dot = el('circle', {
      cx: cfg.cx, cy: cfg.cy, r: 8,
    ...
    // Label above
    g.appendChild(text(label, {
      x: cfg.cx, y: cfg.cy - cfg.r - 4,
      'text-anchor': 'middle', fill: '#383868', 'font-size': 9, 'font-family': 'monospace'
    }));
  }
  ...
  function build5DCluster(g) {
    ...
    const btnR = 11, off = 28;
    // Center
    const cc = el('circle', {
      cx: D5_CX, cy: D5_CY, r: D5_R,
      fill: '#1c1c38', stroke: '#3a3a70', 'stroke-width': 1.5
    });
    g.appendChild(cc);
    refs['d5_center_btn'] = cc;
    g.appendChild(text('●', {
      x: D5_CX, y: D5_CY + 4, 'text-anchor': 'middle',
      fill: '#3a3a68', 'font-size': 9, 'pointer-events': 'none'
    }));
    // Cardinal directions
    for (const [dir, dx, dy] of dirs) {
      const cx = D5_CX + dx * off, cy = D5_CY + dy * off;
      const btn = el('circle', { cx, cy, r: btnR, fill: '#1c1c38', stroke: '#3a3a70', 'stroke-width': 1.5 });
      g.appendChild(btn);
  ...
  function buildSwitch(g) {
    ...
    // Track
    g.appendChild(el('rect', {
      x, y: cy - trackH / 2, width: trackW, height: trackH, rx,
      fill: '#0d0d22', stroke: '#2a2a55', 'stroke-width': 1.5
    }));
    // Labels F / N / S
    const positions = [[-1, 'F'], [0, 'N'], [1, 'S']];
    for (const [pos, lbl] of positions) {
      g.appendChild(text(lbl, {
        x: cx + pos * (trackW / 2 - 12), y: cy - trackH / 2 - 4,
        'text-anchor': 'middle', fill: '#3a3a68', 'font-size': 9
      }));
    }
    // Indicator nub
    const nub = el('circle', {
      cx: cx, cy, r: trackH / 2 - 2,
  ...
  function buildScreen(g) {
    ...
    g.appendChild(el('rect', { x, y, width: w, height: h, rx, fill: '#080814', stroke: '#222244', 'stroke-width': 1.5 }));
    // Screen label
    g.appendChild(text('SCREEN', {
      x: x + 8, y: y + 14,
      fill: '#1e1e40', 'font-size': 9, 'font-family': 'monospace'
    }));
  ...
  function buildTriggerBars(g) {
    ...
    const lb = el('rect', {
      x: LTRIG.x + 2, y: LTRIG.y + 2,
      width: 0, height: LTRIG.h - 4, rx: LTRIG.rx - 2,
      fill: '#4060c0', opacity: 0.55
    });
    g.appendChild(lb);
    refs['ltrig_bar'] = lb;
    ...
    const rb = el('rect', {
      x: RTRIG.x + 2, y: RTRIG.y + 2,
      width: 0, height: RTRIG.h - 4, rx: RTRIG.rx - 2,
      fill: '#4060c0', opacity: 0.55
    });
    g.appendChild(rb);
    refs['rtrig_bar'] = rb;
  ...
  function setActive(id, active, isPico = false) {
    ...
    if (active) {
      ...
      btn.setAttribute('fill', dim ? '#1e1e3a' : (isPico ? '#221a10' : '#1c1c38'));
      btn.style.filter = '';
    }
  }
  ...
  function setSwitch(pos) {
    ...
    nub.setAttribute('fill', pos === 'f' ? '#805010' : pos === 's' ? '#205080' : '#3a5090');
  }
  ...
  function setScreenElements(elements) {
    ...
    const cols = 4, chipW = 130, chipH = 22, gapX = 10, gapY = 6;
    const startX = x + 14, startY = y + 22;
    elements.slice(0, 16).forEach((elem, i) => {
      const col = i % cols, row = Math.floor(i / cols);
      const ex = startX + col * (chipW + gapX);
      const ey = startY + row * (chipH + gapY);
      const chip = el('rect', { x: ex, y: ey, width: chipW, height: chipH, rx: 5, fill: '#131330', stroke: '#252550' });
      g.appendChild(chip);
      const t = text(elem.name || elem.id, {
        x: ex + 8, y: ey + 15, fill: '#5050a0', 'font-size': 10,
        'font-family': 'monospace'
      });
      g.appendChild(t);
      const ledC = el('circle', { cx: ex + chipW - 10, cy: ey + chipH / 2, r: 4, fill: elem.state ? '#3dd68c' : '#222240', stroke: '#333365' });
      g.appendChild(ledC);
    });
  }
  ...
  function update(rcState, picoConn) {
    ...
    if (halfBar) {
      halfBar.setAttribute('width', (shutterHalf && !rtrigFull) ? refs['rtrig_bar_maxw'] / 2 : 0);
    }

...
const ConfigEditor = (() => {
  ...
  const INPUT_LABELS = {
    ...
    pico_shutter_half: { label: 'Shutter ½ (Pico)',  source: 'pico', type: 'button' },
    pico_shutter_full: { label: 'Shutter full (Pico)', source: 'pico', type: 'button' },
  ...
  function initProfileHandlers() {
    ...
    document.getElementById('btn-new-profile')?.addEventListener('click', () => {
      ...
      `, async () => {
        ...
        const clone = document.getElementById('new-profile-clone').value || null;
        if (!name) { window.App?.toast('Enter a profile name', 'error'); return false; }
    ...
    document.getElementById('btn-delete-profile')?.addEventListener('click', async () => {
      const sel = document.getElementById('profile-select');
      const name = sel?.value;
      if (!name || name === 'default') { window.App?.toast('Cannot delete default profile', 'error'); return; }
    ...
    document.getElementById('import-file-input')?.addEventListener('change', async (ev) => {
      ...
      let data;
      try { data = JSON.parse(text); } catch { window.App?.toast('Invalid JSON', 'error'); return; }
  ...
  function actionBadgeHtml(mapping) {
    ...
    let detail = '';
    if (act === 'vjoy_axis')   detail = `${mapping.axis || '?'} ${mapping.invert ? '(inv)' : ''}`;
  ...
  function renderMappings() {
    ...
    document.getElementById('btn-save-mappings')?.addEventListener('click', saveAllMappings);
  }
  ...
  function buildActionForm(inputId, currentMapping) {
    ...
    let typeOptions = ACTION_TYPES.filter(t => {
      ...
      return true;
    }).map(t => `<option value="${t.value}" ${curAct === t.value ? 'selected' : ''}>${t.label}</option>`).join('');
  ...
  function collectMappingFromForm(actionType) {
    ...
    if (actionType === 'vjoy_axis') return {
      action: 'vjoy_axis',
      axis:   document.getElementById('af-axis')?.value || 'X',
      invert: document.getElementById('af-invert')?.checked ?? false,
      dead_zone: parseFloat(document.getElementById('af-dz')?.value ?? '0.02'),
    };
    if (actionType === 'vjoy_button') return {
      action: 'vjoy_button',
      button: parseInt(document.getElementById('af-btn-id')?.value ?? '1'),
    };
    if (actionType === 'key') return {
      action: 'key',
      keys: (document.getElementById('af-keys')?.value || '').split(',').map(s => s.trim()).filter(Boolean),
    };
    if (actionType === 'system') return {
      action: 'system',
  ...
  function openMappingEditor(inputId) {
    ...
    window.Modal?.show(`Map: ${INPUT_LABELS[inputId]?.label || inputId}`,
      buildActionForm(inputId, currentMapping),
      async () => {
        const actionType = document.getElementById('action-type-sel')?.value || 'none';
    ...
    setTimeout(() => {
      document.getElementById('action-type-sel')?.addEventListener('change', e => {
  ...
  function openElementEditor(elemId) {
    ...
    window.Modal?.show(`Element: ${_elemName(elem, elemId)}`,
      `<div class="form-row">
        <label>Name</label>
      ...
      <div class="form-row">
        ...
        <select id="action-type-sel">${
          ACTION_TYPES.map(t => `<option value="${t.value}" ${(currentMapping.action||'none')===t.value?'selected':''}>${t.label}</option>`).join('')
        }</select>
      </div>
      <div id="action-fields"></div>`,
      async () => {
        const actionType = document.getElementById('action-type-sel')?.value || 'none';
        const newMapping = collectMappingFromForm(actionType);
        const newName    = document.getElementById('elem-name')?.value.trim() || elemId;

        // Build the update patch in server format
        const patch = { display_name: newName };
        if (etype === 'slider')      patch.on_change  = newMapping;
        else if (etype === 'button') { patch.on_press = newMapping; patch.on_release = elem.on_release || { action: 'none' }; }
  ...
  async function toggleElementLed(elemId) {
    try {
      ...
      if (_elements[elemId]) _elements[elemId].current_value = result.state ?? result.value;
      renderElements();
    } catch (e) { window.App?.toast(e.message, 'error'); }
  }
  ...
  return { init, loadRegistry, renderAll, loadAll, _getMappings: () => _mappings };
})();
